(function() {
  ...
  dummy_path = d3.svg.line().x(x_from_slider).y(-50).defined(function(d) {
    return d !== null;
  });
  ...
  s_path = function(udaman_name) {
    return d3.select("g#chart_area #path_" + (window.series_to_class(udaman_name)));
  };
  ...
  window.trim_time_series = function(event) {
    slider_extent = $("#line_chart_slider_div").val().map(function(value) {
      return +value;
    });
  ...
  window.multi_line_to_line_and_bar = function(d) {
    ...
    return d3.select("#right_axis_label").text("%Change");
  };
  ...
  window.display_line_and_bar_chart = function(d) {
    ...
    d3.select("#left_axis_label").text(d.axis_name != null ? "" + d.axis_name + " (" + d.units + ")" : "" + d.display_name + " (" + d.units + ")");
    return d3.select("#right_axis_label").text("%Change");
  };
  ...
  window.add_to_line_chart = function(d, axis) {
    ...
    d3.selectAll("g#chart_area path." + y[axis]["class"]).attr("d", function(d) {
      return y[axis].path(d[freq].trimmed_data);
    });
    return d3.select("#" + axis + "_axis_label").text(d.axis_name != null ? "" + d.axis_name + " (" + d.units + ")" : "" + d.display_name + " (" + d.units + ")");
  };
  ...
  window.remove_from_line_chart = function(d, axis) {
    ...
    path.attr("d", function(d) {
      console.log(d);
      console.log(freq);
      if (d[window.old_freq].trimmed_data != null) {
        return dummy_path(d[window.old_freq].trimmed_data);
      }
    });
    update_domain(axis, duration);
    return chart_area.selectAll("path." + y[axis]["class"]).attr("d", function(d) {
      return y[axis].path(d[freq].trimmed_data);
    });
  };
  ...
  window.set_up_line_chart_paths = function(data) {
    return d3.select("g#chart_area").selectAll("path.line_chart_path").data(data).enter().append("path").attr("id", function(d) {
      return "path_" + (window.series_to_class(d.udaman_name));
    }).attr("class", function(d) {
      return "" + (window.series_to_class(d.udaman_name)) + " line_chart_path";
    }).attr("stroke", "#777");
  };
  ...
  window.line_chart = function(container) {
    ...
    svg.append("text").attr("id", "left_axis_label").attr("text-anchor", "start").attr("y", 20).text("Left Label");
    svg.append("g").attr("id", "right_axis").attr("transform", "translate(" + (margin.left + chart_area_width) + "," + margin.top + ")").call(y.right.axis);
    svg.append("text").attr("id", "right_axis_label").attr("text-anchor", "end").attr("x", chart_area_width + margin.left + margin.right - 2).attr("y", 20).text("Right Label");
